...
export default function ManageLeads({ refreshKey = 0, onReset, onImportClick }: Props) {
  ...
  const handleRefresh = () => { setSearchInput(""); setActiveSearch(""); setFilterStatus(""); fetchData("", ""); };

  const handleLoadMore = async () => {
    const next = page + 1;
    setLoadingMore(true);
    try {
      const res = await leadsAPI.getAll({ q: activeSearch || undefined, status: filterStatus || undefined, page: next, limit: PAGE_SIZE });
  ...
  const handleExport = () => {
    ...
    window.open(`${base}/leads/export${filterStatus ? `?status=${filterStatus}` : ""}`, "_blank");
  };
  ...
  return (
    <div style={{ background: "#f9fafb", minHeight: "100vh" }}>
      ...
      {/* Error banner */}
      {fetchError && (
        <div style={{ margin: "12px 16px 0", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
          <AlertCircle size={15} color="#dc2626" />
          <span style={{ fontSize: 13, color: "#dc2626", flex: 1 }}>{fetchError}</span>
      ...
      <div className="ml-stats">
        {[
          { label: "Total",     value: stats?.totalLeads ?? total,               color: "#111827", bg: "#fff",    border: "#e5e7eb", filter: "" },
          { label: "Good",      value: stats?.byStatus.GOOD_LEAD_FOLLOW_UP ?? 0, color: "#15803d", bg: "#f0fdf4", border: "#bbf7d0", filter: "GOOD_LEAD_FOLLOW_UP" },
          { label: "Sales",     value: stats?.byStatus.SALE_DONE ?? 0,           color: "#1d4ed8", bg: "#eff6ff", border: "#bfdbfe", filter: "SALE_DONE" },
          { label: "Not Dialed",value: stats?.byStatus.DID_NOT_CONNECT ?? 0,     color: "#6b7280", bg: "#f9fafb", border: "#e5e7eb", filter: "DID_NOT_CONNECT" },
          { label: "Bad",       value: stats?.byStatus.BAD_LEAD ?? 0,            color: "#dc2626", bg: "#fef2f2", border: "#fecaca", filter: "BAD_LEAD" },
        ].map(s => (
      ...
      <div className="ml-table-card">
          ...
          <div className="ml-search-row">
            <div className="ml-search">
              <input
                ...
                style={{ fontSize: 13, padding: "7px 12px", outline: "none", color: "#374151", border: "none", background: "#fff", minWidth: 0, flex: 1 }}
              />
              <button onClick={handleSearch} style={{ background: "#1f2937", padding: "7px 12px", color: "#fff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", flexShrink: 0 }}>
                <Search size={13} />
              </button>
            </div>
            <button onClick={handleRefresh} title="Refresh" style={{ padding: "7px 8px", color: "#9ca3af", border: "1px solid #e5e7eb", borderRadius: 8, background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", flexShrink: 0 }}>
              <RefreshCw size={13} />
            </button>
          </div>
        </div>
            ...
            <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse", minWidth: 700 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #f3f4f6", background: "#f9fafb" }}>
                  {["LEAD NAME","EMAIL","CONTACT","COMPANY","STATUS",""].map(h => (
                    ...
                    >
                      <td style={{ padding: "12px 14px", fontWeight: 500, color: "#111827", whiteSpace: "nowrap" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis" }}>{row.name || "—"}</span>
                        </div>
                      </td>
                      <td style={{ padding: "12px 14px", color: "#6b7280", maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.email || "—"}</td>
                      <td style={{ padding: "12px 14px", color: "#6b7280", whiteSpace: "nowrap" }}>
                        {row.mobile_without_country_code ? `${row.country_code || "+91"} ${row.mobile_without_country_code}` : "—"}
                      </td>
                      <td style={{ padding: "12px 14px", color: "#6b7280", whiteSpace: "nowrap", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis" }}>{row.company || "—"}</td>
                      <td style={{ padding: "12px 14px", whiteSpace: "nowrap" }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          ...
                          <select value={row.crm_status || ""} disabled={isUpd || !row.id} onChange={e => row.id && handleStatusUpdate(row.id, e.target.value)}
                            style={{ padding: "3px 6px", borderRadius: 6, fontSize: 11, fontWeight: 600, background: st.bg, color: st.color, border: `1px solid ${st.border}`, cursor: "pointer", outline: "none", opacity: isUpd ? 0.6 : 1 }}>
                      ...
                      <td style={{ padding: "12px 14px", whiteSpace: "nowrap" }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: "flex", gap: 4 }}>
                          <button onClick={() => setSelected(isSel ? null : row)} style={{ padding: "5px 7px", color: "#6b7280", border: "1px solid #e5e7eb", borderRadius: 7, background: isSel ? "#f3f4f6" : "#fff", cursor: "pointer", display: "inline-flex", alignItems: "center" }}>
                            <Eye size={12} />
                          </button>
                          <button onClick={() => row.id && handleDelete(row.id)} disabled={isDel || !row.id} style={{ padding: "5px 7px", color: "#ef4444", border: "1px solid #fecaca", borderRadius: 7, background: "#fff", cursor: "pointer", display: "inline-flex", alignItems: "center", opacity: isDel ? 0.5 : 1 }}>
                            <Trash2 size={12} />
                          </button>
                        </div>
                      </td>
        ...
        <div style={{ padding: "12px 20px", borderTop: "1px solid #f3f4f6", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 12, color: "#9ca3af" }}>Showing {leads.length} of {total}</span>
          {hasMore ? (
            ...
            total > 0 && <span style={{ color: "#d1d5db", fontSize: 12 }}>All {total} loaded</span>
          )}
        </div>
      </div>
      ...
      {/* Lead detail drawer */}
      {selected && (
        <div className="ml-drawer" style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: 340, background: "#fff", borderLeft: "1px solid #e5e7eb", zIndex: 40, overflowY: "auto", boxShadow: "-4px 0 24px rgba(0,0,0,0.1)" }}>
          <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid #f3f4f6", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "#fff", zIndex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, color: "#111827", fontSize: 14 }}>{selected.name || "Unknown"}</div>
            ...
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid #f3f4f6" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {CRM_STATUS_OPTIONS.map(s => (
                  <button key={s.value} onClick={() => selected.id && handleStatusUpdate(selected.id, s.value)} disabled={updating === selected.id}
